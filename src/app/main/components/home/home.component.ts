...
export class HomeComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.searchPosts([], '', null, 0, 10);
    this.ownerControl.valueChanges.pipe(tap((value) => {
      clearTimeout(this.timeoutSearch);
      this.timeoutSearch = setTimeout(() => {
        this.autoCompleteOwnerOptions = value !== null &&
          value !== undefined &&
          value.length > 1 ? this.userService.getItemsLike(value, this.USER_LIMIT) : of([]);
      }, 500);
    })).subscribe();
  }
  ...
  searchPosts(tags: string[], owner: string, status: boolean | null, pageIndex: number, pageSize: number): void {
    this.postService.getAllByCriteria(tags, owner, status, pageIndex, pageSize).subscribe(postPage => {
  ...
  onSubmit(): void {
    ...
    this.searchPosts(this.tagsControl.value, this.ownerControl.value, this.statusControl.value, 0, 10);
  }
  ...
  pageChanged(event: PageEvent): void {
    ...
    this.searchPosts(this.tagsControl.value, this.ownerControl.value, this.statusControl.value, event.pageIndex, event.pageSize);
  }
